...
class Customs:
    def __init__(self, gate_num, duration, arrive_interval, check_interval):
        ...
        self.car_num = 0
        self.arrival_interval = arrive_interval
        self.check_interval = check_interval
        
    def wait_time_acc(self, n):
        self.total_wait_time += n
        
    def total_time_add(self, n):
    ...
    def free_gate(self, i):
        if self.gates[i] == 1:
            ...
            raise ValueError("clear gate error: gate" + i)
    ...
    def statistics(self):
        print("Simulation " + str(self.duration) + "mimutes, for" + str(len(self.gates)) + " gates.")
        print(self.car_num, "cars pass the customs")
        print("Average waitting time:", self.tital_wait_time/self.car_num)
        print("Average passing time:", self.total_used_time/self.car_num)
        i = 0
        while not self.waitline.is_empty():
            self.waitline.dequque()
            i += 1
        print(i, "cars are in waitting line")
...
def Arrive(Event):
    ...
    if i is not None:
        ...
        Leave(time + randint(*self.check_interval)), i, car, customs)
        
class Leave(Event):
    def __init__(self, leave_time, gate_num, car, customs):
        super.__init__(self, leave_time, customs)
        self.car = car
    ...
    def run():
        ...
        cumtoms.free_gate(self.gate_num)
        customs.car_count_1()
...
if __name__ == "__main__":
    ...
    car_check_time(3, 5)
    cus = Customs(3, 480, car_arrival_interval, car_check_time)
    cus.simulate()
